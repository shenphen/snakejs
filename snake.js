...
Vector.prototype.isSame = function (vector) {
	return this.x == vector.x && this.y == vector.y;
};

Vector.prototype.isInsideArray = function (arr) {
	for(var i = 0; i < arr.length; i++) {
		if(this.isSame(arr[i])) return true;
	}
	return false;
};
Vector.prototype.indexOfArray = function (arr) {
	for(var i = 0; i < arr.length; i++) {
...
function Snake(){
	...
	this.alive = false;
	this.tail = [new Vector(centerX    , centerY), new Vector(centerX - 1, centerY),
				 new Vector(centerX - 2, centerY)];
	this.direction = RIGHT;
	// this.refreshTime = 150;
...
Snake.prototype.update = function(){
	...
	if(newHead.isSame(this.needless)) {
		...
		var shown = true;
		if(shown)console.log("koniec gry");
	}
	...
	if(!this.throughWalls &&(head.x >= COL_NUM || head.x < 0 || head.y >= ROWS_NUM || head.y < 0)) this.alive = false;
	...
	for (i = 0; i < this.eated.length ; ++i){
		var elem = this.eated[i];
		if (!elem.isInsideArray(this.tail)) {
			this.tail.push(this.eated.splice(i, 1)[0]);
		}
	}
...
CanvasDisplay.prototype.drawElement = function (displayVector, tailVector) {
...
CanvasDisplay.prototype.drawElementIndex = function (tailArray, index) {
...
CanvasDisplay.prototype.firstDraw = function (snake) {
...
CanvasDisplay.prototype.clearEnd = function (lastElement) {
	this.cx.fillStyle = this.bgColor;
	this.cx.fillRect(lastElement.x * PIX_SIZE, lastElement.y * PIX_SIZE, PIX_SIZE, PIX_SIZE);
};

CanvasDisplay.prototype.drawApples = function (snake) {
...
CanvasDisplay.prototype.draw = function (snake) {
...
Menu.prototype.switchButtons = function (value) {
...
Menu.prototype.enableContinue = function (value) {
...
Menu.prototype.toggleView = function () {
	if (this.hidden) {
		this.node.className = "show";
	}
	else {
...
Menu.prototype.toggleSettings = function () {
	if (this.settings) {
...
function Game(snake, display) {
	...
	var pic = new Image(); //document.getElementById("snake-grid-norm");
...
Game.prototype.enableKeys = function () {
